...
To run this example we execute:
...
```
python sklearn_elasticnet_wine/train.py 0.0098 0.4
...
We can optionally organize runs into *experiments*, which group together runs for a specific task. We can create an experiment using the mlflow experiments CLI, with `mlflow.create_experiment()`, or using the corresponding REST parameters. Let's create a new experiment with the CLI:
...
A new experiment can be seen in the UI (left tab), and a new dir `1` is added into the `mlruns` directory.
...
Run `mlflow ui` in the same current working directory as the one which contains the mlruns directory and navigate our browser to http://localhost:5000.
...
specifies that the project has the library dependencies located in a Conda environment file called `conda.yaml` and it has one entry point which consists of a command and a set of configurable parameters (`alpha` and `l1_ratio` in this case) to pass to that command at runtime.
...
If we go to the UI we see this run is added, but the source is `sklearn_elasticnet_wine` (instead of `train.py` like in the previous runs).

Alternatively we can run the project specifying the experiment. The experiment created by default has the id 0 so we can run the project with the command:

```
mlflow run -e main --experiment-id 0 sklearn_elasticnet_wine -P alpha=0.11
```
